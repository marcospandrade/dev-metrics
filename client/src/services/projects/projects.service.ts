...
async function getProjects(integrationServerId: string) {
  try {
    const { data: apiData } = await api.get<GenericHttpResponse<Project[]>>(
      `/integration-server/projects/${integrationServerId}`,
    );
...
async function syncProject(projectId: string) {
  try {
    const { data } = await api.post<GenericHttpResponse<void>>(
      `/integration-server/re-sync-project`,
      {
        projectId,
      },
    );
...
async function getAllCustomFields(projectId: string) {
  try {
    const { data } = await api.get<GenericHttpResponse<AtlassianCustomField[]>>(
      `/integration-server/get-all-issue-fields/${projectId}`,
    );
...
async function registerCustomFields(payload: RegisterCustomFieldsDto) {
  try {
    const { data } = await api.post<GenericHttpResponse<void>>(
      'integration-server/register-custom-fields',
      payload,
    );
