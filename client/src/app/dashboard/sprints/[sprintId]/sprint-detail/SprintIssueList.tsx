...
export function SprintIssueList({ sprintIssues }: Readonly<SprintIssueListProps>) {
  function extractStoryPointField(issue: Issue) {
    const storyPointField = issue.project.customFields.find((customField) =>
      Boolean(customField.isStoryPointField),
    );
          ...
          <tr className="hover:bg-gray-50" key={sprintIssue.id}>
            ...
            <td className="p-4">
              <Typography
                ...
                className="flex items-center gap-2 font-normal leading-none opacity-70"
              >
                {sprintIssue.issue.customFields?.[
                  extractStoryPointField(sprintIssue.issue) ?? ''
                ] ?? 'N/A'}
              </Typography>
            </td>
            <td className="p-4">
