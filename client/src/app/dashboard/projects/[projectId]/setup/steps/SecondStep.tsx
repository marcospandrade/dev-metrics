...
export function SecondStep({ selectedProjectId, relevantCustomFields, handleSetRelevantCustomFields }: Readonly<SecondStepProps>) {
  ...
  async function onChangeSearchField(text: string) {
    ...
    const filteredFields = customFields.filter((field) => field.name.toLowerCase().includes(text.toLowerCase()))
      ...
      <Card className="flex flex-1 mt-5">
        <CardBody>
          <Typography variant="paragraph" className="mb-8">
            Click in the button to open the list of Custom Fields from Jira, then you'll need to select the representative field
          </Typography>

          <Divider />

          <div className="flex flex-1 flex-col gap-y-8 mt-8 px-4">
        ...
        <DialogBody className="h-[40rem] overflow-scroll">
          <div>
            <div className="flex flex-row justify-between">
              ...
              <SearchInput searchInputPlaceholder="Search for custom fields here" onChangeSearchInput={onChangeSearchField} />
            </div>
            <List className="grid grid-cols-3">
              {!!selectedField &&
                filteredCustomFields.map((customField) => (
                  <ListItem key={customField.id} className="flex" onClick={() => onSelectCustomField(selectedField, customField)}>
