...
export class SprintIssuesUseCasesService {
            ...
            await this.repository.manager.transaction(async manager => {
                try {
                    ...
                    for (const sprintIssueToCreate of dtos) {
                        let sprint =
                            sprints.get(sprintIssueToCreate.sprintId) ||
                            (await manager.findOne(Sprint, { where: { id: sprintIssueToCreate.sprintId } }));
                        if (!sprint) {
    ...
    async deleteManyByIssueAndSprintIds(dto: RemoveSprintIssueDto) {
        const sprintIssues = await this.repository.findBy({
            issueId: In(dto.issuesList.map(({ id }) => id)),
